<?php
/**
 * Random cms block file
 *
 * This file holds the functions needed for the random cms block
 *
...
/**
 * Prepare random content block for display
 *
 * @param array $options
 * @return array 
 */
function show_random_cms($options)
{
	...
	// Pick random content from the list, if the block preference is so set
	if ($options[1] == TRUE) 
	{
		shuffle($start_list);
	}
 ...
 * @return string 
 */
function edit_random_cms($options) 
{
	$cmsModule = icms::handler("icms_module")->getByDirname('cms');
	include_once(ICMS_ROOT_PATH . '/modules/' . $cmsModule->getVar('dirname') . '/include/common.php');
	$cms_start_handler = icms_getModuleHandler('start', $cmsModule->getVar('dirname'), 'cms');
	
	// Select number of random cms to display in the block
	$form = '<table>';
	$form .= '<tr><td>' . _MB_CMS_RANDOM_LIMIT . '</td>';
	$form .= '<td>' . '<input type="text" name="options[0]" value="' . $options[0] . '"/></td></tr>';
	
	// Randomise the cms? NB: Only works if you do not cache the block
	$form .= '<tr><td>' . _MB_CMS_RANDOM_OR_FIXED . '</td>';
	$form .= '<td><input type="radio" name="options[1]" value="1"';
	if ($options[1] == 1) 
	{
		$form .= ' checked="checked"';
	}
	$form .= '/>' . _MB_CMS_RANDOM_YES;
	$form .= '<input type="radio" name="options[1]" value="0"';
	if ($options[1] == 0) 
	{
		$form .= 'checked="checked"';
	}
	$form .= '/>' . _MB_CMS_RANDOM_NO . '</td></tr>';	
	
	// Show start logos, or just a simple list?
	$form .= '<tr><td>' . _MB_CMS_LOGOS_OR_LIST . '</td>';
	...
	if (icms_get_module_status("sprockets"))
	{
		...
		$tagList = array(0 => _MB_CMS_RANDOM_ALL) + $tagList;
		$form .= '<tr><td>' . _MB_CMS_RANDOM_TAG . '</td>';
